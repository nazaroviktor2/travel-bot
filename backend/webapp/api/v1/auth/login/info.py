...
from webapp.schema.auth.login.user import UserDTO
from webapp.utils.auth.jwt import JwtTokenT, jwt_auth


@auth_router.get("/info", response_model=UserDTO)
async def auth_info_handler(
    ...
    try:
        ...
        return ORJSONResponse(
            content=UserDTO.model_validate(user).model_dump(), status_code=status.HTTP_200_OK
        )
